...
In the real world, everything can be thought of as an object. A car, a dog, a person—they are all objects. Each object has:
...
When you override a method in a child class (like `__init__` or `bark`), you might still want to run the code from the parent's version of that method. The `super()` function allows you to call the parent class's method from within the child class.

Its most common use is to call the parent's `__init__` constructor from the child's constructor. This avoids re-writing the initialization logic that the parent already handles.
...
In this example, `super().__init__(name)` runs the `__init__` method from the `Animal` class, which takes care of setting `self.name`. The `Dog` class then only needs to handle its own specific attribute, `self.breed`. This makes the code more maintainable and follows the "Don't Repeat Yourself" (DRY) principle.
...
In Python, abstraction can be achieved using **Abstract Base Classes (ABCs)**. An abstract class is a class that cannot be instantiated and is meant to be inherited by other classes. It forces subclasses to implement specific methods.
...
# shape = Shape() # This will raise an error: TypeError: Can't instantiate abstract class Shape with abstract methods area, perimeter
...
By using abstraction, we define a clear interface (`Shape`) that all subclasses (`Rectangle`, `Circle`, etc.) must follow, ensuring consistency while hiding the specific calculations for each shape.

> **Note:** To define a property in an abstract class, you can use the `@property` decorator.
> ```python
> class Machine(ABC):
>     @property
>     @abstractmethod
...
> **Note:** The difference between `__str__` and `__repr__` is that `__str__` is used for printing the object and `__repr__` is used for returning a string representation of the object. Other than that, they are the same.
...
id_card1 = IDCard("19F9AKS", "ABC", 123456789)
p1 = Person("Alice", 30, id_card1)
p2 = Person("Alice", 30, id_card1)
...
class Rectangle:
	...
	def compute_area1(a, b):
		return a * b
	
	@staticmethod
	def compute_area2(a, b):
		return a * b

r = Rectangle(3,4)
r.compute_area1(3,4) # TypeError: Rectangle.compute_area1() takes 2 positional arguments but 3 were given
r.compute_area2(3,4) # 12
Rectangle.compute_area1(3,4) # 12
Rectangle.compute_area2(3,4) # 12
```
...
#### Maintaining and Modifying Class State 
...
class User:
    ...
    def __init__(self, name):
        self.name = name
        User._increment_user_count()

    @classmethod
    def _increment_user_count(cls):
        cls.total_users += 1
...
class Square(Rectangle):
	...
	@classmethod
	def compute_area(cls, a):
		return super(Square, cls).compute_area(a, a)

Square.compute_area(3) # 9
```
...
#### Computed and Read Only Attributes
...
#### Advanced attribute management logic
