...
**Example:** A generator for infinite squares.
```python
def generate_square():
	i = 0
	while True:
...
gen = generate_square()
first_100_squares = [next(gen) for i in range(100)]
```

**Example:** B generator for fibonacci sequence.
...
**Example:** C generator for db connection with API.
...
def read_items(db: Session = Depends(get_db)):
    ...
    return {"message": "Connection active"}
```
...
### Example: B simple decorator with arguments.
```python
def increase_by_(n):
	def wrapper(f):
		def g(x):
...
### Example: C simple decorator to add logging to a function.
...
### Example: D simple decorator to time a function's execution.
...
Interacting with files is a fundamental task. Python provides a straightforward way to read from and write to files. But first, let's learn how to handle file paths.

### Pathlib Module

The `pathlib` module provides an object-oriented approach to handling filesystem paths. It offers a set of classes to handle filesystem paths with semantics appropriate for different operating systems. It is generally considered more readable and easier to use than the older `os.path` module.

#### Creating Paths

You can create a path object by passing a string to the `Path` class.
...
# Absolute path (example for Windows)
abs_path = Path('C:/Users/user/Documents/file.txt')
```

> **Note:** When writing windows paths as string like `"C:\Users\user\Document\folder"`, python will interprete backslashes as escape characters which will cause error. To overcome this you can use raw strings by writing `r"C:\Users\user\Document\folder"` which will disable escape characters.

#### Path Components

`Path` objects provide easy access to various parts of the path.
...
You can join paths using the `/` operator, which is intuitive and platform-independent.
...
print(file_path) # /home/user/projects/data/raw/input.csv
```
...
`pathlib` simplifies reading and writing text to files directly from the path object.
...
For more complex operations, you can still use `open()` directly on the path object:
...
# Create directory (parents=True creates missing parent dirs, exist_ok=True ignores if it already exists)
...
The `random` module implements pseudo-random number generators for various distributions. It is widely used for simulations, games, and testing.
...
print(random.random()) # Output: 0.345... (random float between 0 and 1)
```

#### `randint`
Returns a random integer between a given range (inclusive).

```python
print(random.randint(1, 10)) # Output: 7 (random integer between 1 and 10)
```
...
print(random.choice(items)) # Output: 'banana' (random element from the list)
```
...
print(cards) # Output: [3, 1, 5, 2, 4] (random order)
```

#### `sample`
Returns a k length list of unique elements chosen from the population sequence. Used for random sampling without replacement.

```python
population = range(100)
print(random.sample(population, 5)) # Output: [12, 45, 2, 99, 34] (5 unique random numbers)
```
