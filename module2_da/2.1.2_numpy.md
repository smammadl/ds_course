...
![Broadcasting](../images/2.1_broadcasting.png)
...
*   `np.log()`: Returns the natural logarithm of each element in the array.
...
*   `np.std()`: Returns the standard deviation of the array.
*   `np.var()`: Returns the variance of the array.
...
# Get the mean of elements along each column
print(np.mean(my_2d_array, axis=0))
# Output: [2.5 3.5 4.5]

# Get the mean of elements along each row
...
# Get the minimum value of elements along each column
print(np.min(my_2d_array, axis=0))
# Output: [1 2 3]

# Get the minimum value of elements along each row
print(np.min(my_2d_array, axis=1))
# Output: [1 4]
```
...
*   `np.dot()`: Performs a dot product of two arrays.
...
# Verify the result: A * A_inv = I
print(my_matrix @ inverse_matrix)
# Output:
# [[1. 0.]
#  [0. 1.]]
