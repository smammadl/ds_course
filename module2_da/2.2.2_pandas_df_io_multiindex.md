...
### Getting inner levels

Use `xs` (cross-section) method to get inner levels of a MultiIndex.
```python
d10 = d9.xs('alice', level=1, axis=0, drop_level=False)
```

>You can pass the level name (e.g., `level='People'`) instead of the integer position. You can also pass exact index names like `d9.xs(('London','alice'), level=(0,1), axis=0, drop_level=False)`
