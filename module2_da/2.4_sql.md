...
Similar to `df.head(10)`, SQL uses `LIMIT`. You can also skip a specific number of rows using `OFFSET`, which is often used for pagination.
...
# Skip first 5 rows, then get the next 10 (rows 5 to 15)
df.iloc[5:15]
```

> **Note**: `LIMIT` and `OFFSET` are usually the last clauses in a query.
...
| Operation | SQL | Pandas |
| :--- | :--- | :--- |
| **Concatenate** | `CONCAT(first, ' ', last)` or `first \|\| ' ' \|\| last` | `df['first'] + ' ' + df['last']` |
...
| **Length** | `LENGTH(col)` or `CHAR_LENGTH(col)` | `df['col'].str.len()` |
| **Substring** | `SUBSTRING(col FROM 1 FOR 3)` or `SUBSTR(col, 1, 3)`| `df['col'].str[0:3]` |
| **Left/Right** | `LEFT(col, 3)` / `RIGHT(col, 3)`| `df['col'].str[:3]` / `df['col'].str[-3:]` |

> **Note**: `||` returns `NULL` if any input is `NULL`, whereas `CONCAT` ignores `NULL`s (treats them as empty strings).
...
| **Ceiling** | `CEIL(col)` or `CEILING(col)` | `np.ceil(df['col'])` |
| **Floor** | `FLOOR(col)` | `np.floor(df['col'])` |
| **Rounding** | `ROUND(col, 2)` | `df['col'].round(2)` |
| **Square Root** | `SQRT(col)` or `\|/ col` | `np.sqrt(df['col'])` |
| **Power** | `POWER(col, 2)` or `col ^ 2` | `df['col'] ** 2` |
...
| :--- | :--- | :--- |
| **Extract Part** | `EXTRACT(YEAR FROM col)` or `DATE_PART('year', col)` | `df['col'].dt.year` |
| **Date Trunc** | `DATE_TRUNC('month', col)` | `df['col'].dt.to_period('M')` |
| **Difference** | `col2 - col1` or `AGE(c2, c1)` | `df['col2'] - df['col1']` |
| **Current Date** | `CURRENT_DATE` | `pd.Timestamp.now().date()` |
| **Current Timestamp** | `NOW()` or `CURRENT_TIMESTAMP` or `CLOCK_TIMESTAMP()` | `pd.Timestamp.now()` |
| **Add/Sub** | `col + INTERVAL '1 day'` or `col + MAKE_INTERVAL(days => 1)` | `df['col'] + pd.Timedelta(days=1)` |

> **Notes**: `MAKE_INTERVAL` is a PostgreSQL function that creates an interval from a list of arguments. 
> `AGE` is a PostgreSQL function that returns a human-readable interval (e.g., "2 years 3 months"). For simple day differences, you can just subtract two dates.
> `DATE_PART` is a PostgreSQL function that returns a part of a date.
> `DATE_TRUNC` is a PostgreSQL function that truncates a date to a given unit.
> The difference between `NOW()` and `CURRENT_TIMESTAMP` is that `NOW()` returns the current timestamp with the time zone of the database, while `CURRENT_TIMESTAMP` returns the current timestamp with the time zone of the client. `CLOCK_TIMESTAMP` returns the current timestamp with the time zone of the client.
...
**Pandas Equivalent:**
Pandas doesn't have a direct equivalent to `ROLLUP` or `CUBE` in a single command, but you can achieve similar results using `pivot_table` with `margins=True` or by concatenating multiple `groupby` results.

**1. Equivalent to ROLLUP (Grand Total):**
```python
# Creates a total row at the bottom
df.pivot_table(index='city', aggfunc='size', margins=True, margins_name='Total')
```

**2. Equivalent to CUBE/GROUPING SETS (Manual Construction):**
```python
# Group by country + city
group1 = df.groupby(['country', 'city']).size().reset_index(name='count')

# Group by country only
group2 = df.groupby('country').size().reset_index(name='count')
group2['city'] = 'ALL' # Fill missing level

# Grand total
group3 = pd.DataFrame({'country': ['ALL'], 'city': ['ALL'], 'count': [len(df)]})

# Combine all levels
result = pd.concat([group1, group2, group3]).sort_values(['country', 'city'])
```
...
products[products['price'] > avg_price][['product_name', 'price']]
```
...
**Pandas Equivalent:**
```python
# Calculate avg salary per department and filter
dept_avg = employees.groupby('department_id')['salary'].transform('mean')
employees[employees['salary'] > dept_avg][['employee_name', 'salary', 'department_id']]
```
...
### Using Multiple CTEs
You can define multiple CTEs in a single query by separating them with a comma. This allows you to break down complex logic into sequential, readable steps.
...
region_totals = orders.groupby('region')['amount'].sum().reset_index(name='total_sales')

# Step 2: Filter for top regions
top_regions = region_totals[region_totals['total_sales'] > 10000][['region']]

# Step 3: Merge back to original data to get orders from those regions
result = orders.merge(top_regions, on='region')
```
...
### Understanding Window Frames (`ROWS BETWEEN`)

The `ROWS BETWEEN` clause defines the "frame" or subset of rows used for the calculation relative to the current row.
...
**Examples:**
- `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`: Cumulative total from the start up to now (default for `ORDER BY`).
- `ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING`: The row before, the current row, and the row after (3-row moving window).
- `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`: All rows in the partition (Grand total for the group).
...
**Analyst Use Case:** Creating temporary tables or views.
...
Manages the data within the schema objects.
- **`SELECT`**: Retrieve data (The primary focus of data analysis).
...
Manages the changes made by DML statements to ensure data integrity.
