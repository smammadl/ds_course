...
For simple groupings these methods are equivalent
...
Similarly you can aggregate multiple columns at once.
...
For more complex aggregation, the `aggregate` method could be more handy.
...
> **Note:** You can rename the output of the aggregated columns.
> 
> ```python
> df.groupby('key').agg(
>     dataX= ('data1', 'min'),
...
For more complex transformation, you can define a your own function.
...
| GroupBy Method | What func receives as an argument | What func must return | 
| --- | --- | --- |
| .transform(func) | "Series (one column, one group)" | A Series of the same length (or a scalar to broadcast) |
| .agg(func) | "Series (one column, one group)" | A single scalar value |
| .filter(func) | "DataFrame (all columns, one group)" | A single True or False |
| .apply(func) | "DataFrame (all columns, one group)" | "A DataFrame, Series, or scalar (highly flexible)" |
