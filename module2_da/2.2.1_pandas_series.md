...
> **Note:** In fact you can convert a `Series` to a numpy array or even list, dict etc.
> 
> - `s.values` returns the values of the `Series` as a NumPy array.
> - `s.to_numpy()` returns the values of the `Series` as a NumPy array.
> - `s.tolist()` returns the values of the `Series` as a list.
> - `s.to_list()` returns the values of the `Series` as a list.
> - `s.to_dict()` returns the values of the `Series` as a dictionary.
...
### Index
By default, index is simply the rank of the item in the `Series` (starting from `0`). Index of series is both immutable and unique. But you can also set the index labels manually,
...
Slicing with labels is also possible but the last element is inclusive.
...
> **Note:** NaN stands for "Not a Number" and is used to represent missing values. You can use `.isna()` to check if a value is missing, or `.notna()` to check if a value is not missing.
> ```python
> s_total = s3 + s2
> s_total.isna()
> ```
...
> dtype: bool
> ```
> You can later use `.dropna()` to drop the missing values or `.fillna(value)` to fill the missing values with a specific value.
> ```python
> s_total.dropna()
> ```
> 
...
> **Note:** The `.where()` method replaces values where the condition is False. 
>
>```python
>s = pd.Series([1, 2, 3, 4, 5])
>s.where(s > 3, 0)
...
> **Note:** There is also `numpy` `where()` which allows you to specify a different value for both the `True` and `False` cases, making it a flexible option for "if-else" style replacements.
> 
> ```python
> s = pd.Series([1, 2, 3, 4, 5])
