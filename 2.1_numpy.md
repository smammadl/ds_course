...
To get started with NumPy, you first need to import it. The standard convention is to import it with the alias `np`:
...
In this example, the `my_1d_array` (shape `(3,)`) is broadcast across the `my_2d_array` (shape `(2, 3)`). The 1D array is stretched to match the shape of the 2D array, and then the element-wise addition is performed.
